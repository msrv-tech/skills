...
name: web-test
description: >-
  Тестирование 1С именно через опубликованный веб-клиент в браузере Playwright.
  Используй только когда пользователь явно выбирает браузер или web-клиент;
  для нативного или headless UI 1С используй codex-test-bridge.
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
RUN="<skills-root>/web-test/scripts/run.mjs"
...
Сначала примени `test-databases` и выбери разрешённую запись реестра. Используй её URL веб-публикации, если он задан. `.v8-project.json` допустим только для вспомогательного `webUrl`, но не как источник подключения. Если публикации нет, используй `web-publish`; не конструируй URL неизвестной базы по догадке.
...
Если задача требует редких сценариев, расширенных примеров или глубокой диагностики,
прочитай `references/details.md` перед внесением изменений.
