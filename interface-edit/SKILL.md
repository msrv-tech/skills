...
```powershell
powershell.exe -NoProfile -File '<skills-root>/interface-edit/scripts/interface-edit.ps1' -CIPath '<path>' -Operation hide -Value '<cmd>'
```

### JSON mode

```powershell
powershell.exe -NoProfile -File '<skills-root>/interface-edit/scripts/interface-edit.ps1' -CIPath '<path>' -DefinitionFile '<json>'
```
