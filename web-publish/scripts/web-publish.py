...
def main():
    ...
    if not os.path.exists(httpd_exe):
        ...
        try:
            ...
            zip_req = urllib.request.Request(zip_url, headers={
                'User-Agent': 'Mozilla/5.0',
            })
            with urllib.request.urlopen(zip_req, timeout=120) as resp, open(tmp_zip, 'wb') as f:
        ...
        if not zipfile.is_zipfile(tmp_zip):
            print(f'Error: скачанный файл не является ZIP-архивом: {tmp_zip}', file=sys.stderr)
            print('Скачайте Apache вручную: https://www.apachelounge.com/download/')
            sys.exit(1)
