...
3. **Создать объекты** (`/meta-compile` для каждого): справочники, документы, регистры, модули; каждый объект автоматически регистрируется в `Configuration.xml`
4. **Создать подсистемы** (`/subsystem-compile`): дерево подсистем, распределить объекты
5. **Создать роли** (`/role-compile`): права доступа к объектам
6. **Создать формы** (`/form-add` + `/form-compile`): для объектов с UI
7. **Валидация** (`/cf-validate`): проверить всю конфигурацию
